...
export async function getDashboardData() {
  ...
  const thisMonthPurchases = purchases.filter(p => new Date(p.createdAt) >= thisMonth);
  ...
  const thisMonthTotal = thisMonthPurchases.reduce((sum, p) => sum + (p.price || 0), 0);
  const thisMonthOrders = thisMonthPurchases.length;
  const thisMonthClients = new Set(thisMonthPurchases.map(p => p.buyerEmail)).size;

  // Calculate metrics for last month
  const lastMonthTotal = lastMonthPurchases.reduce((sum, p) => sum + (p.price || 0), 0);
  const lastMonthOrders = lastMonthPurchases.length;
  const lastMonthClients = new Set(lastMonthPurchases.map(p => p.buyerEmail)).size;
