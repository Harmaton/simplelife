...
interface AttachmentFormProps {
  initialData: (Chapter & { attachments: ChapterAttachment[] }) | null;
  courseId: string;
  chapterId: string;
}
...
export const AttachmentForm = ({
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      await axios.post(`/api/courses/${chapterId}/chapters/${chapterId}/attachments`, values);
      ...
      {isEditing && (
        <div>
          ...
          <div className="text-xs text-muted-foreground mt-4">
            Agregue todo lo que sus estudiantes puedan necesitar para completar el capitulo.
          </div>
        </div>
      )}
    </div>
